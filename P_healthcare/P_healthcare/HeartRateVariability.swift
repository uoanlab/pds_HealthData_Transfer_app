...
class HeartRateVariability{
    ...
    public func anchoredHeartRateVariability(completion: @escaping (HKQueryAnchor)-> Void){
        //データの種類(心拍変動)
    ...
    public func storeHeartRateVariability(anchor: HKQueryAnchor){
        //UserDefaultsの参照
        ...
        let anchoredObjectQuery = HKAnchoredObjectQuery(type: type, predicate: nil, anchor: anchor, limit: 0) { (query, newSamples, deletedSamples, newAnchor, error) -> Void in
            ...
            guard let samples = newSamples as? [HKQuantitySample], let deleted = deletedSamples else {
                ...
                print("*** Unable to query for heart rate Variability: \(String(describing: error?.localizedDescription)) ***")
                abort()
            }
            // Process the results...
            for sample in samples {
                ...
                //value取得
                let heartRateVariability = sample.quantity.doubleValue(for: unit)
                
                let contents = "{\"__id\": \"\(ID)\",\"type\": \"\(type)\",\"sourceName\": \"\(sourceName)\",\"sourceVersion\": \"\(sourceVersion)\",\"deviceName\": \"\(devicename)\",\"device\": \"\(device)\",\"unit\": \"\(unit)\",\"startDate\": \"\(startDate)\",\"endDate\": \"\(endDate)\",\"value\": \"\(heartRateVariability)\"}"
                print(contents)
