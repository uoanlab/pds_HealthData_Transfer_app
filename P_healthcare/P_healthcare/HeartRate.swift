...
class HeartRate{
    ...
    public func anchoredHeartRate(completion: @escaping (HKQueryAnchor)-> Void){
    ...
    public func storeHeartRate(anchor: HKQueryAnchor){
        print("心拍数を格納します")
        //UserDefaultsの参照
        ...
        let anchoredObjectQuery = HKAnchoredObjectQuery(type: type, predicate: nil, anchor: anchor, limit: 0) { (query, newSamples, deletedSamples, newAnchor, error) -> Void in
            ...
            guard let samples = newSamples as? [HKQuantitySample], let deleted = deletedSamples else {
                ...
                print("*** Unable to query for heart rate: \(String(describing: error?.localizedDescription)) ***")
                abort()
            ...
            for sample in samples {
                ...
                let contents = "{\"__id\": \"\(ID)\",\"type\": \"\(type)\",\"sourceName\": \"\(sourceName)\",\"sourceVersion\": \"\(sourceVersion)\",\"deviceName\": \"\(devicename)\",\"device\": \"\(device)\",\"unit\": \"\(unit)\",\"startDate\": \"\(startDate)\",\"endDate\": \"\(endDate)\",\"value\": \"\(heartRate)\"}"
                print(contents)
