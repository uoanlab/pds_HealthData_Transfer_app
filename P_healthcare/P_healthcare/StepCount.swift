...
class StepCount{
    ...
    public func anchoredStepCount(completion: @escaping (HKQueryAnchor)-> Void){
        //データの種類(歩数)
    ...
    public func storeStepCount(anchor: HKQueryAnchor){
        print("歩数を格納します")
        //UserDefaultsの参照
        ...
        let anchoredObjectQuery = HKAnchoredObjectQuery(type: type, predicate: nil, anchor: anchor, limit: 0) { (query, newSamples, deletedSamples, newAnchor, error) -> Void in
            ...
            guard let samples = newSamples as? [HKQuantitySample], let deleted = deletedSamples else {
                ...
                print("*** Unable to query for step counts: \(String(describing: error?.localizedDescription)) ***")
                abort()
            }
            // Process the results...
            for sample in samples {
                ...
                //value取得
                let stepCount = sample.quantity.doubleValue(for: unit)
                
                let contents = "{\"__id\": \"\(ID)\",\"type\": \"\(type)\",\"sourceName\": \"\(sourceName)\",\"sourceVersion\": \"\(sourceVersion)\",\"deviceName\": \"\(devicename)\",\"device\": \"\(device)\",\"unit\": \"\(unit)\",\"startDate\": \"\(startDate)\",\"endDate\": \"\(endDate)\",\"value\": \"\(stepCount)\"}"
                print(contents)
...
//            let encodedNewAnchor = NSKeyedArchiver.archivedData(withRootObject: newAnchor!, requiringSecureCoding: true)
//            userDefaults.set(newAnchor, forKey: "anchoredHealthData")
            anchoredSC = newAnchor!
//            userDefaults.set(newAnchor, forKey: "anchoredSC")
