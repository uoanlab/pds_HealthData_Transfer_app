...
class ActiveEnergyBurned{
    ...
    public func anchoredActiveEnergyBurned(completion: @escaping (HKQueryAnchor)-> Void){
        //データの種類(アクティブエネルギー)
    ...
    public func storeActiveEnergyBurned(anchor: HKQueryAnchor){
        print("アクティブエネルギーを格納します。")
        //UserDefaultsの参照
        ...
        let anchoredObjectQuery = HKAnchoredObjectQuery(type: type, predicate: nil, anchor: anchor, limit: 0) { (query, newSamples, deletedSamples, newAnchor, error) -> Void in
            ...
            for sample in samples {
                ...
                //value取得
                let activeEnergyBurned = sample.quantity.doubleValue(for: unit)
                
                let contents = "{\"__id\": \"\(ID)\",\"type\": \"\(type)\",\"sourceName\": \"\(sourceName)\",\"sourceVersion\": \"\(sourceVersion)\",\"deviceName\": \"\(devicename)\",\"device\": \"\(device)\",\"unit\": \"\(unit)\",\"startDate\": \"\(startDate)\",\"endDate\": \"\(endDate)\",\"value\": \"\(activeEnergyBurned)\"}"
                print(contents)
...
//            userDefaults.set(newAnchor, forKey: "anchoredHealthData")
            anchoredAEB = newAnchor!
//            userDefaults.set(newAnchor, forKey: "anchoredAEB")
