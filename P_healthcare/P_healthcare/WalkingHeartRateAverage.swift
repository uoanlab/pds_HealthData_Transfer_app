...
class WalkingHeartRateAverage{
    
    public func anchoredWalkingHeartRateAverage(completion: @escaping (HKQueryAnchor)-> Void){
        //データの種類(歩行時平均心拍数)
        let type = HKSampleType.quantityType(forIdentifier: HKQuantityTypeIdentifier.walkingHeartRateAverage)!
        //データの取得準備
    ...
    public func storeWalkingHeartRateAverage(anchor: HKQueryAnchor){
        //UserDefaultsの参照
        ...
        let anchoredObjectQuery = HKAnchoredObjectQuery(type: type, predicate: nil, anchor: anchor, limit: 0) { (query, newSamples, deletedSamples, newAnchor, error) -> Void in
            ...
            guard let samples = newSamples as? [HKQuantitySample], let deleted = deletedSamples else {
                ...
                print("*** Unable to query for walking heart rate average: \(String(describing: error?.localizedDescription)) ***")
                abort()
            }
            // Process the results...
            for sample in samples {
                ...
                //デバイス名取得
                var devicename = ""
                if(sample.device?.name == nil){
                    devicename = "nil"
                }else{
                    devicename = String(describing:sample.device?.name!)
                }
                ...
                //value取得
                let walkingHeartRateAverage = sample.quantity.doubleValue(for: unit)
                
                let contents = "{\"__id\": \"\(ID)\",\"type\": \"\(type)\",\"sourceName\": \"\(sourceName)\",\"sourceVersion\": \"\(sourceVersion)\",\"deviceName\": \"\(devicename)\",\"device\": \"\(device)\",\"unit\": \"\(unit)\",\"startDate\": \"\(startDate)\",\"endDate\": \"\(endDate)\",\"value\": \"\(walkingHeartRateAverage)\"}"
                print(contents)
