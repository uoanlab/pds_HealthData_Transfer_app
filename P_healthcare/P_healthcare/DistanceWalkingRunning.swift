...
class DistanceWalkingRunning{
    ...
    public func anchoredDistWalkingRunning(completion: @escaping (HKQueryAnchor)-> Void){
        //データの種類(走行距離)
    ...
    public func storeDistWalkingRunning(anchor: HKQueryAnchor){
        print("走行距離を格納します")
        //UserDefaultsの参照
        ...
        let anchoredObjectQuery = HKAnchoredObjectQuery(type: type, predicate: nil, anchor: anchor, limit: 0) { (query, newSamples, deletedSamples, newAnchor, error) -> Void in
            ...
            guard let samples = newSamples as? [HKQuantitySample], let deleted = deletedSamples else {
                ...
                print("*** Unable to query for distance walking running: \(String(describing: error?.localizedDescription)) ***")
                abort()
            }
            // Process the results...
            for sample in samples {
                ...
                //value取得
                let distanceWalkingRunning = sample.quantity.doubleValue(for: unit)
                
                let contents = "{\"__id\": \"\(ID)\",\"type\": \"\(type)\",\"sourceName\": \"\(sourceName)\",\"sourceVersion\": \"\(sourceVersion)\",\"deviceName\": \"\(devicename)\",\"device\": \"\(device)\",\"unit\": \"\(unit)\",\"startDate\": \"\(startDate)\",\"endDate\": \"\(endDate)\",\"value\": \"\(distanceWalkingRunning)\"}"
                print(contents)
